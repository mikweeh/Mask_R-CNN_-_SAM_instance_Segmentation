...
ANCHOR_SIZES = ((32,), (64,), (128,), (256,), (512,))  # ((24,), (48,), (96,), (192,), (384,))
...
def easy_train_one_epoch(model, optimizer, data_loader, device, epoch):
    """
    Simplified training function (matching main.py approach).
    """
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch_idx, (images, targets) in enumerate(data_loader):
        ...
        # Forward pass - only use standard Mask R-CNN losses
        loss_dict = model(images, targets)
        losses = sum(loss for loss in loss_dict.values())  # Simple loss sum
...
def generate_training_report(train_losses, val_losses, val_dice_scores, 
    ...
    config_data = [
        ...
        ['Use Focal Dice Loss', str(USE_FOCAL_DICE)],
        ['Dice weight', str(DICE_WEIGHT)],
        ['Anchor sizes', str([x[0] for x in ANCHOR_SIZES])],
        ['Device', str(DEVICE)],
        ['RPN Pre-NMS Top N', str(RPN_PRE_NMS_TOP_N_TRAIN)],
        ['RPN Post-NMS Top N', str(RPN_POST_NMS_TOP_N_TRAIN)],
        ['RPN NMS Threshold', str(RPN_NMS_THRESH)],
    ]
    ...
    for idx, test_image_path in enumerate(test_image_paths):
        ...
        # Add page break every 2 images to avoid overcrowding
        if (idx + 1) % 2 == 0 and idx < len(test_image_paths) - 1:
            story.append(PageBreak())
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Training
        if EASY_TRAIN:
            train_loss = easy_train_one_epoch(
                model, 
                optimizer, 
                train_loader, 
                DEVICE, 
